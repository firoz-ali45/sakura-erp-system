-- ============================================================
-- PRODUCTION: Auto consumption from recipe (BOM).
-- When a production_item has a recipe (by recipe_id or by item_id),
-- fn_execute_production fills production_consumption from recipe_ingredients
-- and allocates RM batches (FIFO) before posting ledger.
-- ============================================================
...
BEGIN
  ...
  -- ---------- 1) Auto-fill production_consumption from recipe where applicable ----------
  FOR v_item IN
  ...
  LOOP
    -- Resolve recipe: use pi.recipe_id or find by FG item_id (active, latest version)
    ...
    IF v_recipe_id IS NOT NULL AND NOT EXISTS (
      ...
      LOOP
        ...
        -- Allocate batches (FIFO by batch_id) from v_inventory_balance
        FOR v_bal IN
          SELECT vb.batch_id,
                 vb.current_qty AS cur_qty,
                 COALESCE(vb.avg_cost, 0) AS avg_cost
          FROM public.v_inventory_balance vb
          WHERE vb.item_id = v_ingredient_item_id
            AND vb.location_id = v_location_id
            AND vb.current_qty > 0
  ...
  -- ---------- 2) RM consumption: post OUT for each production_consumption row ----------
  ...
  -- ---------- 3) FG: for each production_item create batch, fg_batches row, ledger IN ----------
  FOR v_item IN
...
COMMENT ON FUNCTION public.fn_execute_production(uuid, uuid) IS 'Execute production: auto-fill consumption from recipe when present, post RM OUT, create FG batches and post IN; close order.';
