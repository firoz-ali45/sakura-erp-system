-- ============================================================
-- PRODUCTION: Recipe lookup by item_id must include draft recipes.
-- Ensures production always uses recipe when one exists (active or draft).
-- ============================================================
...
BEGIN
  ...
  -- ---------- 1) Auto-fill production_consumption from recipe (linked via recipes table) ----------
  FOR v_item IN
  ...
  LOOP
    -- Resolve recipe: use pi.recipe_id first, else find by FG item_id (active or draft, latest version)
...
COMMENT ON FUNCTION public.fn_execute_production(uuid, uuid) IS 'Execute production: auto-fill consumption from recipe (by recipe_id or item_id, active/draft), post RM OUT, create FG batches and post IN.';
