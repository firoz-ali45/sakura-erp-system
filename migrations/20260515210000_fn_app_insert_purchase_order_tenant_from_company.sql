-- Fix: tenant_id must reference tenants.id, not companies.id (was copying v_company).
...
COMMENT ON FUNCTION public.fn_app_insert_purchase_order(uuid, uuid, text, bigint, text, uuid, text, date, timestamptz, numeric, numeric, text, numeric, numeric, text) IS
  'Insert purchase_orders for anon/custom auth; company_id from user/context, tenant_id from companies.tenant_id.';
