...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='purchase_orders' AND table_schema='public') THEN
        CREATE TABLE purchase_orders (
            ...
            po_number TEXT UNIQUE NOT NULL, -- Foodics: reference (e.g., PO-000330)
            supplier_id UUID REFERENCES suppliers(id),
            supplier_name TEXT, -- Foodics: supplier (e.g., "Sahet Alwan Factory (مصنع ساحة الالوان)")
            destination TEXT, -- Foodics: destination (e.g., "Main Warehouse")
            status TEXT DEFAULT 'pending', -- Foodics: status (e.g., "Closed")
            business_date DATE, -- Foodics: business_date (e.g., 25-11-2025)
            order_date TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            expected_date TIMESTAMP WITH TIME ZONE,
            total_amount NUMERIC(10, 2) DEFAULT 0,
            vat_amount NUMERIC(10, 2) DEFAULT 0,
            notes TEXT, -- Foodics: notes (usually "-" or empty)
            created_by UUID,
        ...
        RAISE NOTICE 'Table purchase_orders created successfully';
    ELSE
        RAISE NOTICE 'Table purchase_orders already exists, adding missing columns...';
    END IF;
END $$;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='purchase_orders' AND table_schema='public') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='purchase_orders' AND column_name='supplier_id' AND table_schema='public') THEN
            ...
            RAISE NOTICE 'Column supplier_id added';
        END IF;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='purchase_order_items' AND table_schema='public') THEN
        CREATE TABLE purchase_order_items (
            ...
            po_number TEXT, -- Foodics: reference (e.g., PO-000330) - for quick lookup
            supplier_name TEXT, -- Foodics: supplier (e.g., "Al-Mobdeoun (المبدعون)")
            item_id UUID,
            item_name TEXT, -- Foodics: name (e.g., "Information Sticker small/1*1")
            name_localized TEXT, -- Foodics: name_localized (e.g., "استيكر معلومات صغير Information Sticker small/1*1")
            item_sku TEXT, -- Foodics: sku (e.g., "sk-19445")
            quantity NUMERIC(10, 2) NOT NULL DEFAULT 0, -- Foodics: quantity (e.g., 13560)
            unit TEXT, -- Foodics: unit (e.g., "Pcs", "Litter", "kg")
            unit_price NUMERIC(10, 2) DEFAULT 0,
            total_amount NUMERIC(10, 2) DEFAULT 0, -- Foodics: total_cost (e.g., 176.86957 SAR)
            quantity_received NUMERIC(10, 2) DEFAULT 0, -- Foodics: quantity_received (e.g., 13560 or 0)
        ...
        RAISE NOTICE 'Table purchase_order_items created successfully';
    ELSE
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='purchase_order_items' AND table_schema='public') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='purchase_order_items' AND column_name='purchase_order_id' AND table_schema='public') THEN
            ALTER TABLE purchase_order_items ADD COLUMN purchase_order_id UUID REFERENCES purchase_orders(id) ON DELETE CASCADE;
            RAISE NOTICE 'Column purchase_order_id added to purchase_order_items';
        END IF;
