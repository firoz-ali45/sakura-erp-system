-- Fix PO Items Received Quantities Tracking for BIGINT ID Type
-- Your purchase_orders.id is BIGINT (int8), so this script creates functions for BIGINT
...
-- Step 3: Update the main PO quantities function to also update item-level quantities (BIGINT version)
...
-- Step 4: Update trigger function to use BIGINT (if it exists)
...
-- Step 5: Update trigger function for grn_inspection_items
...
-- Step 6: Recreate triggers for grn_inspections table (BIGINT version)
...
-- Step 7: Recreate triggers for grn_inspection_items table
...
-- Step 8: Initialize existing PO item quantities (CRITICAL - Updates all existing POs)
DO $$
DECLARE
    po_record RECORD;
...
BEGIN
    ...
    FOR po_record IN SELECT id FROM purchase_orders ORDER BY id LOOP
        BEGIN
            -- This function updates both item-level AND PO-level quantities
            PERFORM update_po_received_quantities(po_record.id);
            updated_count := updated_count + 1;
            
            IF updated_count % 10 = 0 THEN
                RAISE NOTICE 'Updated % POs so far...', updated_count;
            END IF;
...
-- Step 9: Verify the functions and triggers were created
DO $$
BEGIN
    RAISE NOTICE '✅ PO item received quantity tracking system installed successfully!';
    RAISE NOTICE '   - Function: update_po_item_received_quantities(po_id BIGINT)';
    RAISE NOTICE '   - Function: update_po_received_quantities(po_id BIGINT)';
    RAISE NOTICE '   - Trigger functions updated for BIGINT type';
    RAISE NOTICE '   - Triggers recreated and will automatically update PO quantities when GRN changes';
END $$;

-- Step 10: Test query to verify item-level quantities (optional)
