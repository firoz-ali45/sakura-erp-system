-- ============================================================================
-- SAKURA ERP - MASTER DOCUMENT FLOW FIX
-- ============================================================================
-- Version: 1.0.0
-- Date: 2026-01-26
-- Purpose: Complete fix for PR → PO → GRN → Purchasing document flow
-- ============================================================================
...
RAISE NOTICE '';
RAISE NOTICE '╔════════════════════════════════════════════════════════════╗';
RAISE NOTICE '║  SAKURA ERP - DOCUMENT FLOW ARCHITECTURE FIX              ║';
RAISE NOTICE '║  SAP S/4HANA Style Implementation                         ║';
RAISE NOTICE '╚════════════════════════════════════════════════════════════╝';
RAISE NOTICE '';

-- ============================================================================
-- SECTION 1: DOCUMENT FLOW HELPER FUNCTIONS
-- ============================================================================

DO $$ BEGIN RAISE NOTICE '>>> Section 1: Creating Document Flow Functions...'; END $$;

-- Helper function to insert document flow
...
BEGIN
    ...
    IF v_flow_id IS NOT NULL THEN
        ...
        SET source_number = p_source_number,
            target_number = p_target_number,
...
-- Helper for BIGINT IDs (PO uses BIGINT, document_flow uses UUID)
...
-- ============================================================================
-- SECTION 2: DOCUMENT FLOW TRIGGERS
-- ============================================================================

DO $$ BEGIN RAISE NOTICE '>>> Section 2: Creating Document Flow Triggers...'; END $$;

-- Trigger: PR → PO document flow on linkage
...
-- Trigger: GRN → Purchasing Invoice document flow
...
-- ============================================================================
-- SECTION 3: PO RECEIVING STATUS FUNCTION
-- ============================================================================

DO $$ BEGIN RAISE NOTICE '>>> Section 3: Creating PO Receiving Status Function...'; END $$;
...
-- ============================================================================
-- SECTION 4: PR QUANTITY TRACKING TRIGGERS
-- ============================================================================

DO $$ BEGIN RAISE NOTICE '>>> Section 4: Creating PR Quantity Tracking...'; END $$;

-- Update PR item quantities when linkage created
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        SET 
            ...
            status = CASE 
                ...
                ELSE status
            END,
            is_locked = CASE WHEN v_total_ordered >= quantity THEN TRUE ELSE is_locked END,
            updated_at = NOW()
        WHERE id = NEW.pr_item_id;
        
        -- Update PR header
        SELECT SUM(COALESCE(quantity_ordered, 0)), SUM(quantity)
        ...
        SET 
            status = CASE 
                WHEN v_pr_total_ordered >= v_pr_total_qty THEN 'fully_ordered'
...
-- ============================================================================
-- SECTION 5: VIEWS FOR PR-PO TRACKING
-- ============================================================================

DO $$ BEGIN RAISE NOTICE '>>> Section 5: Creating PR-PO Tracking Views...'; END $$;

-- PR Summary with Linked POs
...
-- ============================================================================
-- SECTION 6: VIEWS FOR PO RECEIPT TRACKING
-- ============================================================================

DO $$ BEGIN RAISE NOTICE '>>> Section 6: Creating PO Receipt Tracking Views...'; END $$;
...
SELECT 
    ...
    COALESCE(grn_summary.total_received, 0) AS received_qty,
    poi.quantity - COALESCE(grn_summary.total_received, 0) AS remaining_qty,
    CASE 
        WHEN poi.quantity > 0 THEN ROUND((COALESCE(grn_summary.total_received, 0) / poi.quantity * 100), 2)
        ELSE 0 
    END AS received_percentage,
    CASE 
        WHEN COALESCE(grn_summary.total_received, 0) >= poi.quantity THEN 'fully_received'
        WHEN COALESCE(grn_summary.total_received, 0) > 0 THEN 'partial_received'
        ELSE 'not_received'
    END AS item_receiving_status,
    COALESCE(grn_summary.grn_count, 0) AS grn_count,
    poi.total_amount AS ordered_value,
    COALESCE(grn_summary.total_received, 0) * poi.unit_price AS received_value
FROM purchase_order_items poi
JOIN purchase_orders po ON po.id = poi.purchase_order_id
LEFT JOIN (
    SELECT gii.item_id, gi.purchase_order_id,
           SUM(gii.received_quantity) AS total_received,
           COUNT(DISTINCT gi.id) AS grn_count
    FROM grn_inspection_items gii
    JOIN grn_inspections gi ON gi.id = gii.grn_inspection_id
    WHERE gi.status IN ('passed', 'approved', 'conditional')
      AND (gi.deleted = FALSE OR gi.deleted IS NULL)
    GROUP BY gii.item_id, gi.purchase_order_id
) grn_summary ON grn_summary.item_id = poi.item_id AND grn_summary.purchase_order_id = poi.purchase_order_id
...
DO $$ BEGIN RAISE NOTICE '>>> Section 7: Creating Helper Functions...'; END $$;

-- Get PR linked POs
CREATE OR REPLACE FUNCTION get_pr_linked_pos(p_pr_id UUID)
RETURNS TABLE (
    po_id BIGINT, po_number TEXT, supplier_name TEXT,
    po_status TEXT, receiving_status TEXT,
    converted_qty NUMERIC, item_count BIGINT,
    po_total NUMERIC, po_date TIMESTAMPTZ
) AS $$
BEGIN
    RETURN QUERY
    SELECT ppl.po_id, ppl.po_number, po.supplier_name,
           po.status, po.receiving_status,
           SUM(ppl.converted_quantity), COUNT(DISTINCT ppl.pr_item_id),
           po.total_amount, po.order_date
    FROM pr_po_linkage ppl
    JOIN purchase_orders po ON po.id = ppl.po_id
    WHERE ppl.pr_id = p_pr_id AND ppl.status = 'active'
    GROUP BY ppl.po_id, ppl.po_number, po.supplier_name, po.status, po.receiving_status, po.total_amount, po.order_date
...
RETURNS TABLE (
    history_id UUID, previous_status TEXT, new_status TEXT,
    changed_by_name TEXT, change_reason TEXT, change_date TIMESTAMPTZ
) AS $$
BEGIN
    RETURN QUERY
    SELECT psh.id, psh.previous_status, psh.new_status,
           psh.changed_by_name, psh.change_reason, psh.change_date
...
RETURNS TABLE (
    flow_id UUID, source_type TEXT, source_number TEXT,
    target_type TEXT, target_number TEXT,
    flow_type TEXT, direction TEXT, created_at TIMESTAMPTZ
) AS $$
BEGIN
    RETURN QUERY
    SELECT df.id, df.source_type, df.source_number,
           df.target_type, df.target_number, df.flow_type,
           'downstream'::TEXT, df.created_at
    FROM document_flow df
    WHERE (df.source_type = p_doc_type AND (df.source_id = p_doc_id OR df.source_number = p_doc_number))
    UNION ALL
    SELECT df.id, df.source_type, df.source_number,
           df.target_type, df.target_number, df.flow_type,
           'upstream'::TEXT, df.created_at
    FROM document_flow df
...
-- RPC for updating PO quantities
...
DO $$ BEGIN RAISE NOTICE '>>> Section 8: Granting Permissions...'; END $$;

GRANT EXECUTE ON FUNCTION insert_document_flow TO authenticated;
GRANT EXECUTE ON FUNCTION insert_document_flow_bigint TO authenticated;
GRANT EXECUTE ON FUNCTION update_po_receiving_status TO authenticated;
GRANT EXECUTE ON FUNCTION get_pr_linked_pos TO authenticated;
GRANT EXECUTE ON FUNCTION get_pr_status_history TO authenticated;
GRANT EXECUTE ON FUNCTION get_document_flow TO authenticated;
GRANT EXECUTE ON FUNCTION update_po_received_quantities TO authenticated;
...
-- ============================================================================
-- SECTION 9: BACKFILL EXISTING DATA
-- ============================================================================

DO $$ BEGIN RAISE NOTICE '>>> Section 9: Backfilling Existing Data...'; END $$;

-- Backfill PR → PO document flow
INSERT INTO document_flow (source_type, source_id, source_number, target_type, target_id, target_number, flow_type, created_at)
SELECT DISTINCT 'PR', ppl.pr_id, ppl.pr_number,
       'PO', uuid_generate_v5(uuid_ns_oid(), 'PO-' || ppl.po_id::TEXT), ppl.po_number,
       'converted_to', NOW()
FROM pr_po_linkage ppl
WHERE NOT EXISTS (
...
-- Backfill PO → GRN document flow
INSERT INTO document_flow (source_type, source_id, source_number, target_type, target_id, target_number, flow_type, created_at)
SELECT DISTINCT 'PO', uuid_generate_v5(uuid_ns_oid(), 'PO-' || gi.purchase_order_id::TEXT), gi.purchase_order_number,
       'GRN', gi.id, gi.grn_number, 'goods_received', NOW()
FROM grn_inspections gi
WHERE gi.purchase_order_id IS NOT NULL AND gi.purchase_order_number IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM document_flow df
...
-- Backfill GRN → Purchasing document flow
INSERT INTO document_flow (source_type, source_id, source_number, target_type, target_id, target_number, flow_type, created_at)
SELECT DISTINCT 'GRN', pi.grn_id, pi.grn_number,
       'PURCHASING', pi.id, COALESCE(pi.purchasing_number, pi.invoice_number), 'invoiced', NOW()
...
WHERE pi.grn_id IS NOT NULL AND pi.grn_number IS NOT NULL
  AND NOT EXISTS (
    ...
    WHERE df.source_type = 'GRN' AND df.source_number = pi.grn_number
      AND df.target_type = 'PURCHASING' AND df.target_number = COALESCE(pi.purchasing_number, pi.invoice_number)
)
ON CONFLICT DO NOTHING;
...
DO $$ BEGIN RAISE NOTICE '>>> Section 10: Creating Indexes...'; END $$;
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '╔════════════════════════════════════════════════════════════╗';
    RAISE NOTICE '║  ✅ MASTER DOCUMENT FLOW FIX COMPLETED SUCCESSFULLY       ║';
    RAISE NOTICE '╠════════════════════════════════════════════════════════════╣';
    RAISE NOTICE '║  Document Flow Records: %                              ║', LPAD(v_df_count::TEXT, 5);
    RAISE NOTICE '║  PR-PO Linkage Records: %                              ║', LPAD(v_link_count::TEXT, 5);
    RAISE NOTICE '╠════════════════════════════════════════════════════════════╣';
    ...
    RAISE NOTICE '╠════════════════════════════════════════════════════════════╣';
    RAISE NOTICE '║  NEXT: Refresh browser to see updated data               ║';
    RAISE NOTICE '╚════════════════════════════════════════════════════════════╝';
END $$;
