-- ============================================================================
-- SAKURA ERP - COMPLETE ARCHITECTURE FIX
-- ============================================================================
-- Version: 3.0.0
-- Date: 2026-01-26
-- SAP Reference: EBAN → EKKO/EKPO → EKBE → RBKP/RSEG
-- ============================================================================
-- ROOT CAUSE: BIGINT vs UUID mismatch in document_flow joins
-- SOLUTION: Use TEXT polymorphic keys for all cross-table references
-- ============================================================================
...
BEGIN
    IF TG_OP IN ('INSERT', 'UPDATE') THEN
        ...
        SET 
            ...
            quantity_remaining = GREATEST(0, quantity - v_total_ordered),
            po_id = NEW.po_id,
...
SELECT 
    ...
    COALESCE(SUM(COALESCE(pri.quantity_remaining, pri.quantity - COALESCE(pri.quantity_ordered, 0))), 0) AS remaining_qty,
    ...
    END AS ordered_pct,
    CASE 
        WHEN SUM(COALESCE(pri.quantity_remaining, pri.quantity)) <= 0 THEN FALSE
...
BEGIN
    ...
    SET 
        ...
        quantity_remaining = pri.quantity - COALESCE((
            SELECT SUM(converted_quantity) 
            FROM pr_po_linkage ppl 
            WHERE ppl.pr_item_id = pri.id AND ppl.status = 'active'
        ), CASE WHEN pri.po_id IS NOT NULL THEN pri.quantity ELSE 0 END),
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '╔════════════════════════════════════════════════════════════╗';
    RAISE NOTICE '║  ARCHITECTURE FIX COMPLETE                                 ║';
    RAISE NOTICE '╠════════════════════════════════════════════════════════════╣';
    ...
    RAISE NOTICE '║    • trg_grn_item_po_receipt (GRN→PO qty)                 ║';
    RAISE NOTICE '║    • trg_grn_status_po_update (GRN approval)             ║';
    RAISE NOTICE '║    • trg_pr_po_linkage_pr_update (PR qty tracking)       ║';
    RAISE NOTICE '║    • trg_pr_status_history_log (PR status log)           ║';
    RAISE NOTICE '║    • trg_purchasing_invoice_doc (Invoice doc flow)       ║';
    RAISE NOTICE '╠════════════════════════════════════════════════════════════╣';
    ...
    RAISE NOTICE '║    • v_document_flow_timeline                             ║';
    RAISE NOTICE '╠════════════════════════════════════════════════════════════╣';
