...
const iframeSrc = ref('/sakura-accounts-payable-dashboard/payable.html');
...
const handleIframeLoad = () => {
  ...
  setTimeout(() => {
    const iframe = document.getElementById('accounts-payable-frame');
    if (iframe && iframe.contentWindow) {
      ...
      // Trigger data loading (critical fix)
      setTimeout(() => {
...
const handleIframeError = () => {
  loading.value = false;
  console.error('Failed to load Accounts Payable dashboard');
};
