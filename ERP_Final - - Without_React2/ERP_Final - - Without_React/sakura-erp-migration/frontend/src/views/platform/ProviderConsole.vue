  ...
  <div class="min-h-screen bg-[#f0e1cd] p-4 md:p-6">
    <div class="mb-6">
      ...
      <p class="text-sm text-gray-600 mt-1">
        Manage client companies and subscription status from one place.
      </p>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 mb-4">
      <div class="flex items-center justify-between mb-3">
        <h3 class="text-lg font-semibold text-gray-800">Clients / Tenants</h3>
        <button
...
async function loadCompanies() {
  ...
  try {
    ...
    if (e) {
      warning.value = 'Provider-level listing is blocked by current RLS in this auth mode. Use service-role backend for full cross-company control.';
      throw e;
    }
...
async function saveCompany(companyId) {
  ...
  try {
    ...
    error.value = err?.message || 'Failed to update company subscription';
  } finally {
    savingId.value = null;
  }
}
